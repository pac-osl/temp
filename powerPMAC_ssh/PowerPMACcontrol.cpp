...
int PowerPMACcontrol::writeRead(const char *cmd, std::string& response, int timeout){
    ...
    int sem_val; // pac
    sem_getvalue(&sem_writeRead, &sem_val); // pac
    printf("\nPAC DEBUG: In writeRead: Before getting sempahore: sem_writeRead address = %p,  sem_val = %i",&sem_writeRead, sem_val); // pac
    ...
    sem_getvalue(&sem_writeRead, &sem_val); // pac
    printf("\nPAC DEBUG: In writeRead: After getting sempahore: sem_writeRead address = %p,  sem_val = %i",&sem_writeRead, sem_val); // pac
    sleep(1); // pac
    ...
    {   //Semaphore OK
        ...
        printf("\nIn writeRead: releasing semaphore"); // pac
#ifdef WIN32 
		if (!ReleaseSemaphore(ghSemaphore, 1, NULL))
		{
			debugPrint_ppmaccomm("%s : Error releasing Semaphore\n",functionName);
